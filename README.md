...
| Experimental 30-second response briefs on Mac | Opt individual Pi chats into an additional, restricted model request that creates a native reading rail for each newly completed answer. Briefs keep critical caveats visible, link only to exact local slices of the immutable original, and always provide the full unmodified response. Model and thinking choices are independent of the source chat. Requires a matching companion server advertising `response-brief-v1`; there is no generic-agent fallback. See [setup, privacy, limits, and verification](docs/response-briefs.md). |
