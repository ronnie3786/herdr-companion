...
| Floating Mac assistant | Use a compact floating panel to send prompts and follow agent progress and results without keeping the main window in front. Session notifications show cumulative Pi cost in the bottom-right corner, refreshing every 15 seconds. “Cost …” means Pi has not reported a cost yet. Audio controls sit at the top right. |
| Notes | Create notes from the Mac HUD note stack, open note cards, or File → New Note (Shift-Command-N), including when no notes exist. Capture ideas in resizable Mac HUD notes, edit their title and text on iPhone, and use them as context for agents. iPhone saves sync through the companion and detect conflicting edits. |
...
| Recent chats on Mac | Choose Recents from the sidebar clock menu to see recent chats with prominent regular-weight titles and a compact machine, workspace, and tab subtitle. Hover for full context or right-click to open the workspace. Requires no server update. |
