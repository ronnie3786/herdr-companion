...
| First Mate stability and recovery on Mac | A scheduler guardian and persistent hourly sweep detect stale work, assess legitimate waits, nudge stuck workers, and safely continue verified-stopped assignments. Source recovery archives, durable effect receipts, successor acknowledgement, retry limits, and handoff-churn detection prevent blind restart loops. One human request can authorize several ordered stages without repeated confirmation; explicit human decisions and uncertain external effects still stop work. Open **First Mate → Workflow → Stability & recovery** for progress, interventions, checkpoints, and archive references. Requires the matching server/Pi package with `first-mate-reliability-v1`; older components remain compatible but lack these protections. See [behavior, configuration, and limits](docs/first-mate/reliability.md). |
| First Mate gate verification on Mac | Each feature's **Overview** shows a **Verification** section separate from workflow status: Verified, Partially verified, Failed, or Verification unavailable, with the exact tested revision and package-qualified gate set. Managed workers report discovered suite inventories and every gate batch, including failures, errors, skipped suites, and interrupted runs. Earlier passing suites that drop out of the current gate set are named, a partial gate set for the changed packages is labeled **Partially verified** instead of green, and later failures cannot be hidden by selecting a passing subset. Evidence is bound to the current revision, so a new commit or uncommitted change downgrades an old pass and the previous gate set remains visible as history. Legacy features without structured evidence and offline cached data stay explicitly unavailable or last reported and are never promoted to verified. Requires a companion advertising `first-mate-verification-v1`; install and restart the matching companion/Pi package separately (the Mac updater does not install server packages), and the iPhone/iPad build is distributed separately. See [gate verification](docs/first-mate/gate-verification.md). |
...
| First Mate saved links on Mac | Keep the pull request a feature is working on in front of you. **Overview** leads with a prominent **Pull requests** section and **Documents** repeats it above a **Documents / Links** control, so a detected or explicitly saved PR is one click from **Open** or **Copy** without claiming draft, ready, merged, or closed status. The **Links** sub-tab lists pull requests first and other links separately, shows the exact destination, host (including a custom port), query, fragment, and any agent or discovery provenance, and adds a URL with an optional title and PR classification. **Hide**, **Show hidden**, and **Restore** are reversible and survive reconnects, session handoffs, and server restarts. Managed agents and the companion's bounded PR discovery retain exact `github.com` pull request URLs that match the feature's target PR, repository, or ticket evidence. Unrelated automatic discoveries are hidden and remain restorable; explicit user saves are retained. Requires companion 0.46.2b1 for contextual filtering; the companion never calls GitHub or fetches a destination, and links stay private on their owning companion. Opening and copying validate HTTP(S) and use the saved destination, never a repository guess. Requires a companion advertising `first-mate-links-v1`; older servers keep the rest of First Mate and the control explains the update. See [saved links](docs/first-mate/links.md). |
...
| First Mate response feedback on Mac | Every completed text-bearing First Mate assistant response, including older replies, checkpoint summaries, and closed or archived features, gets compact thumbs-up and thumbs-down controls that display their saved rating. Thumbs up saves immediately; thumbs down opens a small editor pinned to that exact response with the starting reasons **Longer than it needed to be**, **Unnecessary message**, and **Incorrect assumption**, multi-selection, a reusable custom reason added with an explicit Add action, and an optional multiline note preserved verbatim up to 4000 characters. Save can submit a negative rating with no reasons; Cancel leaves a saved rating unchanged; a failed save keeps the editor and its content for an explicit retry, while a failed thumbs-up or **Remove rating** keeps the previous label and shows **Try again** (a stale-revision conflict instead offers **Reload and retry**); the editor freezes its inputs while a save is in flight, and drafts stay pinned to the revision they were loaded from until an explicit conflict reload; changing to thumbs up or **Remove rating** clears the saved reasons and note only after the save succeeds. Ratings, reasons, and notes live in the owning companion's private `first-mate.sqlite3`, not in a Mac-only database, and survive companion restarts and Mac reconnects; an offline Mac keeps the last loaded rating readable but disables writes. The editor is scoped to one feature and connection lifecycle, so switching features or reconnecting dismisses it instead of retargeting another response. Feedback is data collection only: it never sends a message, wakes an agent, changes preferences, trains a model, or uploads anything. Requires a companion advertising `first-mate-feedback-v1`; older servers receive no feedback requests and show one upgrade notice, and the Mac updater does not install the separate server package. See [response feedback](docs/first-mate/response-feedback.md). |
| First Mate on iPhone and iPad | One conversation per feature, with a workflow timeline/graph, step-linked agents and documents, and exact saved Pi sessions including handoff history. Opens on **All Machines**, combining every configured host with each card naming its owner, while the host menu still filters to one machine and remembers an explicit choice; creating from the combined view requires choosing a destination host. Archive finished or irrelevant features without changing workflow status or deleting their records; Show Archived makes them available to inspect or unarchive across the hosts in scope. iPhone uses focused sheets; iPad adds a feature sidebar and inspector. Supports system/light/dark appearance and scalable text. Uses the same First Mate state and authenticated API as Mac; archive controls require `first-mate-archive-v1` from the matching companion server. The signed Mac update feed does not install this iOS build. See [mobile behavior and verification](docs/first-mate/ios.md). |
...
| First Mate status colors on Mac | First Mate status indicators reuse the agent session HUD's red, green, and yellow signals so one state reads the same everywhere: **Blocked** is red, **Waiting on your direction** (`awaiting_direction`, shown as **Your direction**, **Needs you**, or a parked **Your turn**) is green like an unread HUD result, and **Working** (running or coordinating) is yellow. The same mapping covers the status badges on Overview, saved sessions, the fleet sidebar, Agents, Workflow, and Chat, plus the Dashboard and Agent view status pills; an explicit blocked status stays red even if an inconsistent payload also sets the parked-turn flag, and every other status keeps its existing treatment. The dark appearance uses the existing HUD tokens exactly, while the First Mate light appearance deepens the same hues so captions and icons stay readable on the badge's color wash. The global palette, iOS, web, attention counting, the orange attention badge, banners, filtering, and execution are unchanged. This is Mac presentation only: no server update or new configuration is needed. |
...
| Pi compaction completion | Mac and iPhone/iPad chats show a clear completion cue when Pi confirms context compaction: the compacting spinner is replaced beside the prompt by a checkmark and **Context compacted** with a readiness line — **Ready for your next message.** when the session is connected and idle, the available steer or follow-up modes while Pi is still working, and an offline/reconnect message otherwise. Manual, automatic-threshold, and overflow compactions all show it, even when no assistant reply follows. The cue stays while you type and after a failed send; the next accepted message dismisses only the composer cue, while the transcript's **Context compacted** entry remains, and opening an already-compacted chat reconstructs the fact from its saved compaction entries. Starting another compaction, changing sessions, or navigating to another branch removes it, and cancellation, failure, settlement, disconnection, or a timeout never produces it. It needs no new server capability: an explicit `session_compact` event or a persisted compaction entry is required, and missing evidence is never guessed from token counts, elapsed time, or a disappearing spinner. The Mac update feed installs only the Mac app; the iPhone/iPad build and companion packages are distributed separately. See [state semantics and verification](docs/compaction-indicators.md). |
...
| Saved HUD chats | On iPhone and iPad, open **Agents → HUD Chats** to search, read, continue, or create the same saved conversations available in the Mac HUD's **Chat history**. Choose the machine that owns the chat; accepted turns and status sync through its companion, and visible chats refresh without resubmitting prompts. Leaving the screen never deletes or stops a saved run. New chats default to the selected machine’s home folder (`~`). Use the HUD folder menu → **Manage folders…** to add or remove custom absolute paths saved separately for each machine on this Mac, then choose a path before sending. Existing chats keep their original folder across clients. Custom-folder submission now reaches and validates the path on the target server; it requires the updated companion advertising `hudChatWorkingDirectory`, and older servers show an upgrade message instead of rejecting an unexplained request. Remote paths are sent unchanged; custom-folder shortcuts and unsent drafts remain local. On Mac, a fresh composer also exposes a new-chat-only **Create in main workspace** checkbox, unchecked by default and absent from existing, restored, and continued conversations. Checked Send uploads the pending attachments into that workspace first, then creates one new tab and pane there on the selected companion and submits the initial prompt and quotes, without a duplicate headless HUD conversation, focus theft, or a later **Continue in agent** step. The designation is chosen once per machine as an exact workspace, never from a label, number, roster position, generic name, or the currently focused workspace, and is stored privately on this Mac by paired machine ID, normalized endpoint, and raw workspace ID: renaming or reordering keeps it, while removal or endpoint replacement asks for a new choice. Every menu entry and selected destination shows the raw workspace ID beside any label, and switching machines reloads its destinations. Unchecked Send keeps the independent bubble behavior described here. A missing workspace, offline host, upload failure, or uncertain create/prompt keeps the draft or the confirmed pane and offers explicit **Open chat**/start-over recovery instead of silently resending or duplicating work: an uncertain create is never replayed automatically because the companion's request-ID cache is memory-only and expires, while only a failure recorded before the create was attempted safely reuses the request ID. Checked creation requires a companion advertising `quick-session-launch-options-v1`; an older server shows upgrade guidance before receiving any new launch fields, and the Mac updater does not install companion packages. Sending from the Mac HUD creates an independent **HUD chat** bubble beneath the orb and immediately frees the composer for another one-off. Each chat keeps its own transcript, draft, attachments, running state, and explicit workspace handoff. Its bubble copies the agent-session bubble's status and metadata presentation: a filled yellow bolt-circle **Running** row while work is in flight, and the conversation's current model and cumulative reported USD cost alternating in the trailing slot on the same synchronized five-second fade. The **HUD chat** header, speech-bubble icon, and title visibility behavior stay, and the previous running-only yellow outline is replaced by ordinary card separation; a completed unread reply keeps its green **Ready** signal and never auto-opens. No machine name is shown. Missing or incomplete metadata stays unknown rather than guessing a model or partial total, and no server update is needed beyond existing `hud-chat-v1` support. Click a bubble to expand its chat; **New chat** or the orb returns to the separate fresh composer. The clock button opens searchable **Chat history** even while other chats run, reusing an already-open conversation. **Remove from HUD** hides a finished bubble without deleting server history. Bubbles and unread state survive relaunch; saved active runs reconnect without resubmitting. Saved-chat browsing and home-folder conversations use existing `hud-chat-v1` support; the custom-folder fix requires a separate companion server update, not just an app update. **New chat** saves rather than deletes the conversation. Chats and their Pi sessions remain indefinitely in private server storage, outside terminal workspaces; **Continue in agent** explicitly promotes the full conversation. Agents can find them with `herdr-hud-chats list`, `search <text>`, and `show <agr_ID>`; with a companion that advertises `chat-tab-colors-v1` and an opted-in publisher, `--scope terminal` on list/search adds terminal chats with read-only tab color and label filters and grouping without changing saved-history behavior. HUD chats use normal Pi tools, skills, extensions, and project context, subject to existing Pi project trust settings—not a read-only or Herdr-sandboxed profile. Requires the updated companion server, CLI, and Pi package; older servers show an upgrade message before new HUD submissions. Contextual note/code questions keep their separate restricted profile. See [HUD history](docs/hud-chat-history.md). |
...
| PR Review on Mac | Open **PR Review** from the left navigator under First Mate (⌘8) and paste a GitHub pull request link. The review is prepared on the machine whose role is `development`: the companion fetches the PR with `gh`, checks it out into its own worktree, parses the diff, opens a tab in a dedicated **PR Reviews** Herdr workspace, and runs the review, explainer-video and utility skills you choose. Files are ranked by AI impact with a single-category filter, a **Hide viewed** toggle and a **Guided** reading order; PR Review, Chat Git, and First Mate Git use one shared syntax-highlighted code renderer with roomier line spacing, red/green rows, gutters, and changed-word emphasis. A short **Why** explanation for the impact rating is visible above each diff, and empty filters keep their controls at the top. Deleted files carry a textual **Deleted** label in the rail and header and start collapsed: selecting one shows its path and removal summary with a **Show deleted content** control instead of all removed lines, and **Hide deleted content** collapses it again. Disclosure is remembered per file in the window for the current review revision and never changes the patch, viewed state, or Ask AI; an explicit line-navigation or highlight request reveals the content. **Ask AI** is available on any selection, sending the file, side, lines, surrounding code and the agents' findings (reference only) to a read-only question profile that verifies against the checkout. A context library holds findings, HTML reports, audio, videos, links and dropped files; Agents and Skills tabs show every run and let you mark skills ran or not run, or add custom skills. Right-click an active review row (selected or not) or the open review's header to **Pop Out into Window**: that review opens in its own resizable window pinned to the same host, so it stays visible while the main window keeps navigating other chats; reopening focuses the existing window, closing is presentation-only, and every active review can have its own window. Reviews are archived, never deleted. The `herdr-pr-review` CLI and `pr-review.*` agent-control actions let agents and tutors drive the same workspace. Requires companion 0.27.0b1 advertising `pr-review-v1` on the review host; older servers show an update message. New Ask AI questions stay in **Saved questions** bubbles below the diff and can be reopened after navigation or relaunch; they remain local to this Mac, pinned to the original host, file and revision. Select code and choose **Add comment** beside **Ask AI** to save a private local comment with its exact multiline Markdown, file, before/after lines, saved revision and code excerpt; the review-wide **Comments** button opens the list from any tab, **Show in diff** returns to the exact saved lines, **Edit** updates the text, **Copy comment** copies only the comment so you can paste it on GitHub yourself, and **Open file in GitHub** opens that file's **Files changed** place. Comments stay on this Mac, are shared by the main and popped-out windows, never sync or post, and keep an **Earlier revision** excerpt after a rebase instead of remapping anchors. Local comments need no server or CLI change beyond the existing `pr-review-v1` review data. Pop-outs, saved bubbles and bundled PR diff rendering need no server support beyond `pr-review-v1`. Companion **0.40.0b1**, installed separately on the review host, adds short-by-default answer instructions and refresh recovery that retains the last usable review, handles rebased heads, and prevents optional viewed sync from breaking review preparation. See [PR Review](docs/pr-review.md). |
...
| Report bugs and request features from the Mac app | Help → **Report a Bug or Request a Feature…** (⌘⌥F) or Settings → General → Feedback opens a sheet for a Bug or Feature request with a final title, a description, and up to six attachments (screenshots or documents via the file picker, drag and drop, or ⌘V). An optional **Smart input** box above the fields accepts plain English or an inline recording: the glowing microphone turns into Stop only while it captures, and stopping sends the audio once to the selected companion's configured transcription service and appends the transcript to the same box. **Draft with AI** starts one bounded, tool-free `issue-report-draft-v1` request on that companion (its validated configured Pi default, thinking Off, at most 60 seconds) and fills both fields, which stay editable; one guarded **Restore previous** action undoes the last generation, and an older companion disables only AI drafting with upgrade guidance. Smart-input text and recordings go to that companion's configured Pi and transcription services and are not published as report content. **Included details** shows the environment fields that accompany the report; machine names, hostnames, and workspace labels are never included. Only **File report** files the final edited title and description word for word as a public GitHub issue. This Mac's companion files it through your authenticated `gh`, falling back to the first connected companion if needed, and hosts attachments as assets of a rolling `issue-attachments` release so images render inline. Leave **Start the automated fix pipeline** on to label the issue `herdr-autofix`. Requires a companion server advertising `issue-reports-v1`; AI drafting also requires `issue-report-draft-v1`, and older servers keep manual reporting with an update message for the unsupported part only. See [smart input](docs/issue-report-smart-input.md). |
...
documents, and remain linked to their exact saved sessions. A request that explicitly
authorizes a sequence of stages can continue between them without another message;
otherwise a completed stage waits for your plain-English direction. Explicit human
gates and uncertain external effects still pause. The timeline and graph show the
...
sub-tabs with the same prominent PR section above them. Add a bounded
`http`/`https` URL with an optional title and PR classification, open or copy it
at its exact saved destination, and reversibly hide or restore it. Recognizable
`github.com` pull request URLs in managed session evidence are retained
automatically, including draft and ready references to the same PR. Saving or
hiding a link never changes workflow status, revision, authorization, or queued
work. This needs the matching companion with `first-mate-links-v1`, installed
and restarted separately; the Mac updater does not deploy server packages. See
[saved links](docs/first-mate/links.md).

On Mac and iOS, each feature's **Overview** reports gate verification separately
from the workflow badge. The **Verification** section shows the current verdict,
the tested revision, and the exact package-qualified gate set, plus every
missing, previously passing dropped, failing, or stale suite. Managed workers
discover the suites of each changed package, record inventories and every gate
batch (failures included), and cite the run IDs in their outcomes. When a stage
completes, the coordinator selects the retained runs that verify it, and the
checkpoint stores that verdict and gate set. If any suite of a changed package
lacks a current passing result, the verdict is **Partially verified**, never
green; an earlier pass cannot establish the current revision, and a later
failure remains visible even when a passing subset is selected. The companion
and Pi extension must be updated and restarted together for
`first-mate-verification-v1`; the Mac updater does not install server packages,
and the iPhone/iPad build is distributed separately. See
[gate verification](docs/first-mate/gate-verification.md).
