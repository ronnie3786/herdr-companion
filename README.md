...
| Comfortable reading on Mac | A charcoal and lavender interface with larger system-font conversation text, bounded reading width, and Quiet chrome navigation. The prompt keeps labeled Attach, Paste code, and Voice actions close at hand, with Terminal keys above the input. Long drafts scroll with the mouse or trackpad after five visible lines. Tool groups are labeled Clanking and stay collapsed on failures, with a failure count in the header. Appearance follows the app text-size preference. No server update is needed. |
| Pi agent conversations | Chat with Pi agents, follow their replies and tool activity, attach files in a compact, horizontally scrolling strip, and choose available models and reasoning settings. On Mac, the chat header shows the machine. Use the star beside its title to add or remove the chat from Starred. Click its title to edit inline (Enter or focus loss saves, Escape cancels). Right-click a sidebar chat or chat header and choose Smart Rename for a contextual title using a separate quick AI run with low thinking and your Quick Chat model setting. Requires the existing headless agent API and a readable Pi conversation. You can also use the pane actions menu, or right-click a sidebar chat, HUD session, or chat header to copy its workspace pane ID. The pane actions menu groups view, control, Pi session, and pane actions, including Reload Pi extensions (`/reload`). Requires Pi and its configured providers. |
...
| Floating Mac assistant | Use a compact floating panel to send prompts and follow agent progress and results without keeping the main window in front. Session notifications show cumulative Pi cost in the bottom-right corner, refreshing every 15 seconds. “Cost …” means Pi has not reported a cost yet. Audio controls sit at the top right. Updated companions allow headless runs for one hour by default; existing timeout overrides remain effective. Set `HERDR_HARNESS_AGENT_TIMEOUT_SECONDS` in the private configuration's `[environment]` table for a limit from 1 to 86,400 seconds. |
| Notes | Create notes from the Mac HUD note stack, open note cards, or File → New Note (Shift-Command-N), including when no notes exist. The Mac HUD starts with one Notes icon: click to expand the list, then click again to minimize it. Capture ideas in resizable note cards, edit their title and text on iPhone, and use them as context for agents. iPhone saves sync through the companion and detect conflicting edits. |
...
| Recent chats on Mac | Choose Recents from the sidebar clock menu for single-line titles with quiet machine/workspace context and explicit Working, Done, or attention status. Hover for full tab context or right-click to open the workspace. Sidebar titles and statuses observe live panes, including Smart Rename and completion updates. Other sidebar categories keep compact labels without the Recents subtitle when switching filters. Requires no server update. |
...
| Sidebar creation | Use the labeled New workspace action, or right-click a workspace heading—including Unread and Starred groups—for New tab. Creation requires a controllable machine. |
