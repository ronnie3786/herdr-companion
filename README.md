...
| Conversation context on Mac | Drag a Pi chat from the sidebar onto another prompt, or right-click it and choose **Add to current prompt**, to add a removable context chip linked by the Herdr workspace ID and current Pi session ID. No transcript is fetched or embedded; the receiving agent is instructed to fetch that session's context from Herdr before handling the current request. Each prompt keeps its own staged references while the Mac app runs. This is Mac-only and needs no server update. |
