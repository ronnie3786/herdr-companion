...
| Report bugs and request features from the Mac app | Help → **Report a Bug or Request a Feature…** (⌘⌥F) or Settings → General → Feedback opens a sheet for a Bug or Feature request with a title, a description sent exactly as written, and up to six attachments (screenshots or documents via the file picker, drag and drop, or ⌘V). **Included details** shows the environment fields that accompany it; machine names, hostnames, and workspace labels are never included. With several paired machines the sheet checks every connected companion at once, defaults to the one that last filed a report for this Mac (otherwise the first available in machine order), and keeps unavailable machines selectable so their reason under the picker explains what to configure; **Check again** rechecks every connected companion without discarding the draft, and a report is never silently redirected if the selected machine disappears. When no companion can file, the sheet names the machines that were checked, calls out disconnected machines that were not checked, and links to the [Code Factory setup guide](docs/code-factory.md). The companion server files a public GitHub issue through your authenticated `gh`, hosting attachments as assets of a rolling `issue-attachments` release so images render inline. Leave **Start the automated fix pipeline** on to label the issue `herdr-autofix`. Requires a companion server advertising `issue-reports-v1`; older servers show an update message. |
