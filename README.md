...
| Notes | Create notes from the Mac HUD note stack, open note cards, or File → New Note (Shift-Command-N), including when no notes exist. The Mac HUD starts with one microphone-sized, 32-point Notes icon: click to expand the list, then click again to minimize it. Capture ideas in resizable note cards, edit their title and text on iPhone, and use them as context for agents. iPhone saves sync through the companion and detect conflicting edits. |
