...
| Pi agent conversations | Chat with Pi agents, follow their replies and tool activity, attach files, and choose available models and reasoning settings. On Mac, click the chat header title to rename it, or right-click a sidebar chat, HUD session, or chat header to copy its workspace pane ID. The pane actions menu groups view, control, Pi session, and pane actions, including Reload Pi extensions (`/reload`). Requires Pi and its configured providers. |
