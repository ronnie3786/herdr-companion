...
With the Mac HUD enabled, press the left and right Command keys together to add a PNG of the frontmost app window to the HUD's separate **New chat** composer. Herdr reads each Command key independently from macOS's combined-session key state. One physical press creates at most one capture; release either key before using the shortcut again.
