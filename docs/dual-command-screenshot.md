...
removed, so the attachment stays readable after the source file is deleted. Dropping never starts a run
...
successful and failed promises, three consecutive drop/remove cycles, durable bytes after the source
is removed, explicit submission, one import per drop, target-state reset, session isolation, and the
existing 4-file, 20 MB, and 21 MB limits. A dropped image is also rendered through the composer's
attachment chip so its thumbnail path is exercised.
