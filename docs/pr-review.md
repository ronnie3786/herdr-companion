...
- Mac interactive (final gate): `xcodebuild -project herdr-harness-mac/herdr-harness-mac.xcodeproj -scheme herdr-harness-mac -destination 'platform=macOS' CODE_SIGNING_ALLOWED=NO test -only-testing:herdr-harness-macUITests/HerdrPRReviewCommentUITests -only-testing:herdr-harness-macUITests/HerdrPRReviewUITests`.
  `HerdrPRReviewUITests` exercises the row and header context menus, two concurrent review windows
  with independent tabs and files, chat navigation with an unsent draft, duplicate-window focus, and
  close-versus-archive. `HerdrPRReviewCommentUITests` drives real diff selection and the bundled
