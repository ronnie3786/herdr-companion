...
struct SmartRenamePaneButton: View {
    ...
    var body: some View {
        if pane.piSemantic?.sessionID?.isEmpty == false {
            Button(model.smartRenamingPaneIDs.contains(pane.id) ? "Renaming…" : "Smart Rename",
                   systemImage: "sparkles") {
                Task { await model.smartRename(pane) }
            }
            .disabled(!model.canControl(machineID: pane.machineID) || model.smartRenamingPaneIDs.contains(pane.id) || model.paneLifecycleBusyIDs.contains(pane.id))
