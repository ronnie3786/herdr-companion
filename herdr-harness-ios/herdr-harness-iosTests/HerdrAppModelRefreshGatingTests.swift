...
struct HerdrAppModelRefreshGatingTests {
    ...
    @Test("Every successful sync stamps last synced at, changed or not")
    func everySyncStampsLastSyncedAt() async throws {
        let defaults = try #require(UserDefaults(suiteName: "HerdrAppModelRefreshGatingTests.lastSynced"))
        defaults.removePersistentDomain(forName: "HerdrAppModelRefreshGatingTests.lastSynced")
        defer { defaults.removePersistentDomain(forName: "HerdrAppModelRefreshGatingTests.lastSynced") }
        ...
        // The second snapshot is identical, so `lastUpdated` must not move —
        // but Settings reads `lastSyncedAt` to answer "is this still talking to
        // my Mac", and that has to be stamped by the boring sync too.
    ...
    func refreshRetainsHudChatsWhilePruningTopologyRoutes() async throws {
        ...
        #expect(model.workspacePath == [.hudChats])
    }
