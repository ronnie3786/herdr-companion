...
- **Session bubbles:** Titles use their natural one- or two-line height. The model name and cost fade together across all bubbles every five seconds beside the status, using one shared clock. New or revealed rows join the current phase without resetting it; panel and scroll heights follow the actual rows. Each bubble shows the chat name in bold, an emoji followed by italic activity, and a separate status with its own icon. Live activity comes from Pi's current work. A short AI topic summary is the fallback when there is no current activity; it never replaces the chat name.
