...
- **Mark Unread:** Right-click a sidebar chat for a persistent local green checkmark reminder. Reading the chat clears it, without restoring HUD notifications or changing server agent status.
- **Response attachments:** The chat timeline no longer shows orphaned “Other session attachments” without their original response.
- **Notes polish:** Ask, Tidy with AI, and Take action now live in a labeled Actions menu. Title and body defaults are another point larger. Cursors and placeholders use readable ink. The header omits New note; use the stack or File menu instead. Top action tooltips still work while another app is active.
