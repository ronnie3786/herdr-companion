...
final class HerdrAppModel {
    ...
    func endPiSessionAndClosePane(in pane: HerdrPane) async {
        ...
        let routeKey = selectedPaneID == pane.id
            ? beginQuickPaneRoute(machineID: pane.machineID, requestID: requestID) : nil
        defer { if let routeKey { cancelPendingPaneRoute(routeKey) } }
        toastMessage = "Ending Pi and keeping the tab…"
        do {
            let result = try await client.retirePiPane(pane, requestID: requestID)
            try await refresh(machineID: pane.machineID, using: client, showSpinner: false, expectedGeneration: generation)
            guard generation == connectionGeneration else { return }
            if let routeKey {
                completeQuickPaneRoute(routeKey, paneID: MachineScopedID.compose(machineID: pane.machineID, rawID: result.nextPaneID))
            }
            toastMessage = result.warnings.first ?? "chat closed — tab kept open"
        } catch {
            // An uncertain request can have created a shell or closed the old
            // pane. Reconcile, but never fall back to DELETE or repeat /quit.
            try? await refresh(machineID: pane.machineID, using: client, showSpinner: false, expectedGeneration: generation)
            guard generation == connectionGeneration else { return }
            toastMessage = nil
            errorMessage = error.localizedDescription
