...
(() => {
  ...
  function renderDetail(){
    ...
    log.innerHTML=d.messages.filter(m=>['user','human','assistant'].includes(m.role)&&m.visibility!=='background').map(m=>`<article class="message ${humanMessage(m)?'user':''}"><span class="avatar" aria-hidden="true">${humanMessage(m)?'You':'FM'}</span><div class="message-main"><div class="message-meta"><strong>${humanMessage(m)?'You':'First Mate'}</strong><time>${escape(date(m.created_at))}</time>${m.status==='queued'?'<small>Queued</small>':''}</div><div class="prose">${messageContent(m)}</div></div></article>`).join('')||empty('Ready for your direction','Tell First Mate what this feature should achieve.');
