...
Companion/server package for the PR Review improvements in macOS 0.37.0-beta.1. Existing native and web API contracts remain compatible; no new configuration keys are required.
...
/path/to/new-runtime/bin/python -m pip install ./herdr_companion-0.37.0b1-py3-none-any.whl
/path/to/new-runtime/bin/herdr-server --help
```

The wheel includes web assets and the Pi package. Keep the configured authentication and state locations. This release does not remotely install anything or restart active services. The Mac Sparkle update does not install this package.
