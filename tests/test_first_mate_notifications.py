...
class FirstMateNotificationTests(unittest.TestCase):
    ...
    def test_unknown_dispatch_and_exhausted_recovery_notify_once_each(self):
        for event_type in ('assignment.dispatch_unknown', 'assignment.recovery_exhausted', 'coordinator.interrupted'):
        ...
        self.assertEqual(len(self.calls), 4)  # initial stage plus three distinct failures
        self.assertEqual(sum(call['text'] == 'Execution needs recovery direction.' for call in self.calls), 3)
